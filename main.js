      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            .find({ $and: [{ "founded_year": 2004 }, { "founded_month": 2 }] }, { "name": 1, _id: 0 })
            ...
            db.collection('companies')
            .find({"name": "Facebook"}, { "name": 1, "number_of_employees": 1, _id: 0 })
            .toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            ...
            db.collection('companies')
            .find({ $and: [{"name": "Facebook"}, {"relationships.is_past": false} ]}, { "relationships.person.first_name": 1, "relationships.person.last_name": 1, _id: 0 })
            .toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            ...
            db.collection('companies')
            .find({"name": "Facebook"}, { "competitions.competitor.name": 1, _id: 0 })
            .toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
